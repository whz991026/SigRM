...
The raw data was directly downloaded from GEO and aligned to hg38 reference genome using hisat2. The candidate m6A sites (corresponding to mutated C within the DRACH motifs) were detected using Samtools (default setting). The m6A and non-modification signals (number of mutated and unmutated sequences with respect to each candidate sites) were counted using GenomicAlignments R package. The Information of a total of 2371331 sites was extracted for 1382 cells, including 991 testing cells and 391 negative control which has mutated YTH domain. We excluded sites with an unexpected error rate exceeding 1% and a median read count below 10, resulting in 510,554 retained candidate m6A sites, which was utilized for the subsequent real data analysis and methodology tests.
...
# Pre-process
We get 3 objects, one is the frequency of A,C,T,g,and other, the others are the gene expression and SNP.
...
Combine them to get the methylated reads count and unmethylated reads count. Strand_plus(T is methylated reads count, C is unmethylated reads count).Strand_minus(A is methylated reads count, G is unmethylated reads count). The rest are the error reads count.
```{r,eval=FALSE}


frequency_all <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_2/origin/frequency_all.rds")


testSNP_all <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_2/origin/SNP_all_7374.rds")
...
saveRDS(error_reads_list,"error_reads.rds")
```
...
#library(ggplot2)
methylation_rate <- readRDS("~/SIGMR_single_cell/data/SMART_3/preprocess2/methylation_rate.rds")

discard_cell <- readRDS("~/SIGMR_single_cell/data/SMART_3/preprocess2/discard_cell.rds")
...
error_reads_list_1 <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess1/error_reads_list_73_control.rds")

error_reads_list_2 <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess1/error_reads_list_73_treatment.rds")


error_reads_list_3 <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess1/error_reads_list_74_treatment.rds")
...
setwd("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess2")
...
#### preprocess
```{r,eval=FALSE}
frequency_all_processed <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess2/frequency_all_processed.rds")
...
path <-  paste0("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/results_ks/res",num_cell_control)
setwd(path )
...
setwd(path)
saveRDS(index_nona,"index_nona.rds")
...
ks_list_function <- function (x,index_x,y,index_y){
  ...
  return(ks_list)
   ...
   index_nona <- readRDS("~/SIGMR_single_cell/data/SMART_3/preprocess2/index_nona_all.rds")
...
expression_logTPM <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/preprocess2/expression_TPM.rds")
...
path <-  paste0("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/results_ks/res",num_cell_control,"sim")
setwd(path )
saveRDS(control_index,"control_index.rds")
...
```{r,eval=FALSE}
kstest_data_random_2 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res2/kstest_data_origin.rds")
kstest_data_random_4 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res4/kstest_data_origin.rds")
kstest_data_random_8 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res8/kstest_data_origin.rds")
kstest_data_sim_2 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res2sim/kstest_data_origin.rds")
kstest_data_sim_4 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res4sim/kstest_data_origin.rds")
kstest_data_sim_8 <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res8sim/kstest_data_origin.rds")
...
res_SigRM_locfit <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res4sim/res_SigRM_locfit.rds")
...
#library(ggplot2)
original_data <- readRDS("~/SIGMR_single_cell/data/SMART_3/preprocess2/expression_TPM.rds")
...
expression <- readRDS("/home/share/haozhe/SIGMR_single_cell/data/SMART_3/origin/gene_expression_73_control.rds")
...
SNP_all_51w <- readRDS("~/SIGMR_single_cell/data/SMART_3/preprocess2/SNP_all_51w.rds")
...
# load the data
res_all <- readRDS("~/SIGMR_single_cell/data/SMART_3/results_ks/res_all_4_sim/res_list.rds")
...
plot1<- plotmclust(lpsmclust,show_cell_names = F)+theme_bw()+theme(plot.title = element_text(hjust = 0.5),panel.grid.major.x=element_blank(),panel.grid.minor.x=element_blank(),panel.grid.minor.y=element_blank(),panel.grid.major.y=element_blank(),axis.text.x = element_blank())
...
plot2<- ggplot()+
  #geom_point(data =pca_result,aes(x = PC1,y = PC2),size = 1)+ 
  geom_point(data=data_plot,aes(x = PC1, y = PC2,color=phase),size=3)+scale_color_manual(values = c("g1" = "#ffd401","s" = "#00b0eb","g2" = "#e20612","other" = "#C0C0C0") )+
  labs(title = "PCA (Cells)", x = "PC1", y = "PC 2")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),panel.grid.major.x=element_blank(),panel.grid.minor.x=element_blank(),panel.grid.minor.y=element_blank(),panel.grid.major.y=element_blank())#+stat_ellipse(data=data_plot_cluster,  geom = "polygon",level=0.9,     linetype = 1,size=0.5,        aes(x = PC1, y = PC2,fill=cluster),alpha=0.3,      show.legend = T,segments = 20)
...
plot1 <- plot1+xlab("PC1")+ylab("PC2")
plot2 <- plot2+theme(plot.title =element_blank())
plot3 <- plot2+stat_ellipse(data=data_plot_cluster,level=0.9,     linetype = 1,size=2,        aes(x = PC1, y = PC2,fill=state),alpha=0.5,segments = 50,geom = "polygon")+annotate("text", label = "1", x = center_1[1], y = center_1[2], size = 6, colour = "black")+annotate("text", label = "2", x = center_2[1], y = center_2[2], size = 6, colour = "black")+annotate("text", label = "3", x = center_3[1], y = center_3[2], size = 6, colour = "black")+annotate("text", label = "4", x = center_4[1], y = center_4[2], size = 6, colour = "black")+geom_line(data=center,aes(x = x, y = y),size=1)+geom_line(data=center2,aes(x = x, y = y),size=1)+geom_line(data=center3,aes(x = x, y = y),size=1)+theme(
...
plot_markergene2 <- ggplot(data_plot_2,aes(x = PC1, y = PC2,color=state))+
  #geom_point(data =pca_result,aes(x = PC1,y = PC2),size = 1)+ 
  geom_point()+scale_color_manual(values = c("1" = "#e20612","2" = "#00b0eb","3" = "#ffd401","4"="#C0C0C0") )+
  labs(title = "PCA (Cells)", x = "PC1", y = "PC 2")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),panel.grid.major.x=element_blank(),panel.grid.minor.x=element_blank(),panel.grid.minor.y=element_blank(),panel.grid.major.y=element_blank())
#geom_text(data = s, aes(x = PC1, y = PC2, label = rownames(s)), vjust = -0.5, hjust = -0.5, size = 1)
...
# regulate network
```{r,eval=FALSE}
data <- readRDS("D:/data/SMART_3/preprocess2/data_plot_case_study1.rds")
data <- data[,3:4]
