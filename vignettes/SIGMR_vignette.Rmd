...
### Function base on DESeq2

DESeq2_test() is the function base on the DESeq2 model.
```{r,include=FALSE}
DESeq2_test <- function(meth_control,meth_test,unmeth_control,unmeth_test){
  ...
  for (i in 1:dim(meth_test)[2]){
    ...
    design = data.frame(Reps = rep(c(paste0("Rep", 1:num),"Rep1"),2),
                       Trt = rep(c(rep("control", num),"test"),2), 
                    Meth = c(rep("meth",(num+1)),rep("unmeth", 
                            (num+1))))
    model = ~ Reps + Meth + Trt + Meth*Trt
    # 
...
test_function <- function(n_Sites = 1000, test_num = 200, control_num = 300, 
     min_expression = 0, max_expression = 3, per_me =  per_me,
     prange1=c(0,0.2),p_s_1=0.01,prange2=c(0.25,0.4),prange3=c(0,0.3),p_s_3=0.01,
      remove.false = TRUE){
...
auc_DESeq2_10x <- numeric(dim(data[[2]])[2])
auc_DESeq2_SMART <- numeric(dim(data[[2]])[2])
...
fdr_DESeq2_10x <- numeric(dim(data[[2]])[2])
fdr_DESeq2_SMART <- numeric(dim(data[[2]])[2])
...
TPR_DESeq2_10x <- numeric(dim(data[[2]])[2])
TPR_DESeq2_SMART <- numeric(dim(data[[2]])[2])
...
for (i in 1 : length(res2)){
  ...
  threshold1 <- p.adjust(res4[[i]][[5]],method = "BH")[order(p.adjust(res4[[i]][[5]],
                                                            method = "BH"))][l1]
  threshold2 <-  p.adjust(res4[[i]][[5]],method = "BH")[order(p.adjust(res4[[i]][[5]],
                                                            method = "BH"))][l2]
  

  p2[,i] <-rep(1,length(res4[[i]][[5]])) 
  p2[which(p.adjust(res4[[i]][[5]] ,method = "BH")<=threshold1) ,i] <-0 
  auc_DESeq2_10x[i] <- auc(roc(label,p2[,i]))
  fdr_DESeq2_10x[i]<-FDR_function(per_me,n_Sites,p2[,i],threshold=threshold1)
  TPR_DESeq2_10x[i]<-TPR_function(per_me,n_Sites,p2[,i],threshold=threshold1)
  
  p2[,i] <-rep(1,length(res4[[i]][[5]])) 
  p2[which(p.adjust(res4[[i]][[5]] ,method = "BH")<=threshold2) ,i] <-0 
  auc_DESeq2_SMART[i] <- auc(roc(label,p2[,i]))
  fdr_DESeq2_SMART[i]<-FDR_function(per_me,n_Sites,p2[,i],threshold=threshold2)
  TPR_DESeq2_SMART[i]<-TPR_function(per_me,n_Sites,p2[,i],threshold=threshold2)
  
}

# set table
auc_table <- cbind(c(auc_SIGMR_10x,auc_SIGMR_SMART,auc_single_10x,auc_single_SMART,
                     auc_DESeq2_10x,auc_DESeq2_SMART),
                   c(rep("SIGMR_10x",length(res2)),rep("SIGMR_SMART",length(res2)), rep("single_10x",length(res2)), rep("single_SMART",length(res2)), rep("DESeq2_10x",length(res2)), rep("DESeq2_SMART",length(res2))))

fdr_table <- cbind(c(fdr_SIGMR_10x,fdr_SIGMR_SMART,fdr_single_10x,fdr_single_SMART,
                     fdr_DESeq2_10x,fdr_DESeq2_SMART),
                   c(rep("SIGMR_10x",length(res2)),rep("SIGMR_SMART",length(res2)), rep("single_10x",length(res2)), rep("single_SMART",length(res2)), rep("DESeq2_10x",length(res2)), rep("DESeq2_SMART",length(res2))))

TPR_table <- cbind(c(TPR_SIGMR_10x,TPR_SIGMR_SMART,TPR_single_10x,TPR_single_SMART
                     ,TPR_DESeq2_10x,TPR_DESeq2_SMART),
                   c(rep("SIGMR_10x",length(res2)),rep("SIGMR_SMART",length(res2)), rep("single_10x",length(res2)), rep("single_SMART",length(res2)), rep("DESeq2_10x",length(res2)), rep("DESeq2_SMART",length(res2))))
...
metric <- test_function(n_Sites = n_Sites, test_num = test_num, control_num =
      control_num , min_expression = 0, max_expression = 3, per_me =  per_me ,prange1=c(0,0.2),p_s_1=0.01,prange2=c(0.25,0.4),prange3=c(0,0.3),p_s_3=0.01,
      remove.false = TRUE)
...
The single_test_10x() and single_test_SMART() functions can only determine whether a site is m6A or not, resulting in a binary output matrix or data frame. In contrast, the SIGMR output is a p-value matrix or data frame. To ensure fairness, we set the threshold to detect the same number of m6A sites as single_test_10x() or single_test_SMART(), allowing for a direct performance comparison between SIGMR and these methods.We also compare the performance of the DESeq2 model.

* In the AUC plot, it is evident that the AUC of SIGMR_10x surpasses that of 10x, and the AUC of SIGMR_SMART outperforms SMART, we can also see that it better than the DESeq2 method.
* Regarding the FDR plot, it's clear that the FDR of SIGMR_10x is lower than that of 10x, and the FDR of SIGMR_SMART is lower than SMART, we can also see that it lower than the DESeq2 method.
* In the TPR plot, we observe that the TPR of SIGMR_10x is superior to that of 10x, and the TPR of SIGMR_SMART excels compared to SMART, we can also see that it better than the DESeq2 method.
...
for (i in 1:length(res2)){
  ...
  res3_[[i]][res3[[i]]] <- 1
    ...
    res1_ks[i] <- ks_function(res1[[5]][(round(0.3*n_Sites)+1):round(0.65*n_Sites),i],
                          res1[[5]][(round(0.65*n_Sites)+1):n_Sites,i])
     
    res2_ks[i] <- ks_function(res2_[[i]][(round(0.3*n_Sites)+1):round(0.65*n_Sites)],
                          res2_[[i]][(round(0.65*n_Sites)+1):n_Sites])

  
  
    res3_ks[i] <- ks_function(res3_[[i]][(round(0.3*n_Sites)+1):round(0.65*n_Sites)],
                          res3_[[i]][(round(0.65*n_Sites)+1):n_Sites])
 
    res4_ks[i] <- ks_function(res4[[i]][[5]][(round(0.3*n_Sites)+1):round(0.65*n_Sites)],
                          res4[[i]][[5]][(round(0.65*n_Sites)+1):n_Sites])
    
    res_cluster_ks[i] <- ks_function(res_cluster[[5]][(round(0.3*n_Sites)+1):round(
      0.65*n_Sites),i],res_cluster[[5]][(round(0.65*n_Sites)+1):n_Sites,i])
    
    
    
    res_similarity_ks[i] <- ks_function(res_similarity[[5]][(round(0.3*n_Sites)+1):
                                                          round(0.65*n_Sites),i],res_similarity[[5]][(round(0.65*n_Sites)+1):n_Sites,i])
    
}

kstest_data <- cbind(c(res1_ks,res2_ks,res3_ks,res4_ks,res_cluster_ks,res_similarity_ks),
                     c(rep("SIGMR",length(res2)),rep("10X",length(res2)),
                       rep("SMART",length(res2)),rep("DESeq2",length(res2)),
                       rep("SIGMR_cluster",length(res2)),
...
## Visualization the results
We can observe that the KS statistics for SIGMRtest(), SIGMR_cluster_test(), and SIGMR_similarity_test() are higher than those for DESeq2, single_test_10x() and single_test_SMART(). These results are consistent with our previous findings.
```{r,echo=FALSE}
...
ggplot(kstest_data , aes( y=ks,fill=method,x=method)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) +guides(x = guide_axis(angle = 90))
