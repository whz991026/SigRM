...
TDR_function(): This function calculates the TDR (true discovery rate).
```{r,include=FALSE}
TDR_function <- function(per_me,n_Sites,res,adjust=TRUE,threshold){
  if(adjust==TRUE){
     res <- p.adjust(res,method = "BH",n=length(res))
  }
...
tdr_SIGMR_10x <- numeric(dim(data[[2]])[2])
tdr_SIGMR_SMART <- numeric(dim(data[[2]])[2])
tdr_single_10x <- numeric(dim(data[[2]])[2])
tdr_single_SMART <- numeric(dim(data[[2]])[2])
label=c(rep(0,round(n_Sites*(per_me))),rep(1,round(n_Sites*(1-per_me))))

# calculate auc, fdr, tdr
for (i in 1 : length(res2)){
  ...
  tdr_SIGMR_10x[i]<-TDR_function(per_me,n_Sites,p2[,i],threshold=threshold1)
  ...
  tdr_SIGMR_SMART[i]<-TDR_function(per_me,n_Sites,p2[,i],threshold=threshold2)
  #FDR
  ...
  tdr_single_10x[i] <-TDR_function(per_me,n_Sites,p[,i],threshold=threshold1)
  tdr_single_SMART[i] <-TDR_function(per_me,n_Sites,p_[,i],threshold=threshold2)
}
...
tdr_table <- cbind(c(tdr_SIGMR_10x,tdr_SIGMR_SMART,tdr_single_10x,tdr_single_SMART),
                   c(rep("SIGMR_10x",length(res2)),rep("SIGMR_SMART",length(res2)), rep("single_10x",length(res2)), rep("single_SMART",length(res2))))
...
TDR_metric <- metric[[3]]
colnames(TDR_metric) <- c("TDR","method")
TDR_metric <- as.data.frame(TDR_metric)
TDR_metric $TDR <-as.numeric(TDR_metric$TDR)
...
Visualizing the results of the comparison of performance: AUC, FDR, and TDR.
...
* In the TDR plot, we observe that the TDR of SIGMR_10x is superior to that of 10x, and the TDR of SIGMR_SMART excels compared to SMART.
...
ggplot(TDR_metric , aes( y=TDR,fill=method,x=method)) + 
  geom_boxplot() + ggtitle("Plot of TDR")+
  theme(plot.title = element_text(hjust = 0.5)) 
...
# TDR
tdr_SIGMR_cluster <- numeric(dim(data[[2]])[2])
tdr_SIGMR <- numeric(dim(data[[2]])[2])
...
for (i in 1 : dim(data[[2]])[2]){
  ...
  threshold2 <- p.adjust(res_cluster[[5]][,i],method = "BH")[order(p.adjust(
    ...
    # TDR
    tdr_SIGMR_cluster[i] <- TDR_function(per_me,n_Sites,p2[,i],threshold=threshold2)
    tdr_SIGMR[i] <- TDR_function(per_me,n_Sites,p1[,i],threshold=threshold1)
}
...
tdr_table <- cbind(c(tdr_SIGMR_cluster,tdr_SIGMR),c(rep("SIGMR_cluster",
                    dim(data[[2]])[2]),rep("SIGMR",dim(data[[2]])[2])))

colnames(tdr_table) <- c("TDR","method")
tdr_table <- as.data.frame(tdr_table)
tdr_table $TDR <-as.numeric(tdr_table $TDR)
...
tdr_SIGMR_similarity <- numeric(dim(data[[2]])[2])
...
for (i in 1 : dim(data[[2]])[2]){
  ...
  tdr_SIGMR_similarity[i] <- TDR_function(per_me,n_Sites,p3[,i],threshold=threshold3)
}
...
tdr_table_ <-cbind(tdr_SIGMR_similarity,rep("SIGMR_similarity",dim(data[[2]])[2]))
colnames(tdr_table_) <- c("TDR","method")
tdr_table <- rbind(tdr_table,tdr_table_)
...
* Similarly, the TDR plot shows that clustering and similarity increase the TDR.
...
ggplot(tdr_table , aes( y=TDR,fill=method,x=method)) + 
  geom_boxplot() + ggtitle("Plot of TDR") +
  theme(plot.title = element_text(hjust = 0.5)) 
```
