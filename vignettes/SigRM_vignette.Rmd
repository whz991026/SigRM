...
index_10x <- which(grepl("10x",FDR_metric[,2]))
index_DESeq2 <- which(grepl("DESeq2",FDR_metric[,2]))
index_edgeR <- which(grepl("edgeR",FDR_metric[,2]))
index_SigRM <- which(grepl("SigRM",FDR_metric[,2]))
index_single <- which(grepl("single",FDR_metric[,2]))

FDR_metric[index_single,2] <- "Bullseye"
FDR_metric[index_edgeR,2] <- "edgeR"
FDR_metric[index_DESeq2,2] <- "DESeq2"
FDR_metric[index_SigRM,2] <- "SigRM"

FDR_metric$platform <- 0
FDR_metric$platform <- 0
FDR_metric[index_10x,4] <- "10x Genomics"
FDR_metric[-index_10x,4] <- "SMART-seq2"


plot_sim_1 <- ggplot(FDR_metric, aes( y=FDR,color=shape2,x=Method)) +
  geom_violin(width = 1, scale = "width", size = 0.2) +
  geom_boxplot(width = 0.6, position = position_dodge(1),       
               outlier.shape = NA, cex = 0.4)+facet_wrap(vars(platform), scales = "free")




plot_sim_2 <- ggplot(FDR_metric, aes( y=FDR,color=Method,x=shape2)) +
  geom_violin(width = 0.9, scale = "width", size = 0.2) +
  geom_boxplot(width = 0.6, position = position_dodge(0.9),         
               outlier.shape = NA, cex = 0.4)+facet_wrap(vars(platform), scales = "free")+labs("Method")+xlab(TeX("Variance Level ($\\beta$)"))
